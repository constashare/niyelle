...
//    Stream functions
	...
	{
		// Following:
		$query = 'SELECT user_id, name, email FROM users WHERE users_id IN (SELECT follow_id FROM follows WHERE user_id = ?)';
		$following = DB::select($query, [$user->id]);
		foreach($following as $key => $value) {
			$query = 'SELECT id FROM users_follows WHERE user_id = ? AND follow_id = ? LIMIT 1';
			$following[$key]->follow_id = DB::select($query, [$user->id, $value->id])[0]->id;
		}
		// Not Following:
		$query = 'SELECT id, name, email FROM users WHERE id
        NOT IN (SELECT follow_id FROM users_follows WHERE user_id = ?)';
